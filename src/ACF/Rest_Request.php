...
/**
 * Extends the ACF Rest Request class so it will support our rest api endpoints.
 *
 * We had to copy the entire class because of the base class's use of private properties and methods
 */
class Rest_Request {
	/**
	...
	private function build_supported_routes() {
		...
		$base_route = "/{$namespace}/resources";
		...
		foreach ( get_post_types( array( 'show_in_rest' => true ), 'objects' ) as $post_type ) {
			...
			$this->supported_routes[] = "{$base_route}/(?P<rest_base>{$rest_base})";
			$this->supported_routes[] = "{$base_route}/(?P<rest_base>{$rest_base})/(?P<id>[\d]+)";

			if ( post_type_supports( $post_type->name, 'revisions' ) ) {
				$this->supported_routes[] = "{$base_route}/(?P<rest_base>{$rest_base})/(?P<id>[\d]+)/(?P<child_rest_base>revisions)";
				$this->supported_routes[] = "{$base_route}/(?P<rest_base>{$rest_base})/(?P<id>[\d]+)/(?P<child_rest_base>revisions)/(?P<child_id>[\d]+)";
			}

			if ( 'attachment' !== $post_type->name ) {
				$this->supported_routes[] = "{$base_route}/(?P<rest_base>{$rest_base})/(?P<id>[\d]+)/(?P<child_rest_base>autosaves)";
				$this->supported_routes[] = "{$base_route}/(?P<rest_base>{$rest_base})/(?P<id>[\d]+)/(?P<child_rest_base>autosaves)/(?P<child_id>[\d]+)";
			}
		}
		...
		// Add user routes.
		$this->supported_routes[] = "{$base_route}/(?P<rest_base>users)";
		$this->supported_routes[] = "{$base_route}/(?P<rest_base>users)/(?P<id>[\d]+)";
		$this->supported_routes[] = "{$base_route}/(?P<rest_base>users)/me";

		// Add comment routes.
		$this->supported_routes[] = "{$base_route}/(?P<rest_base>comments)";
		$this->supported_routes[] = "{$base_route}/(?P<rest_base>comments)/(?P<id>[\d]+)";
	}
	...
	private function set_object_types() {
		...
		if ( 'users' === $base ) {
			...
			$this->object_sub_type = 'user';
		} elseif ( 'comments' === $base ) {
			$this->object_type     = 'comment';
			$this->object_sub_type = 'comment';
	...
	private function set_http_method() {
		...
		if ( ! empty( $_SERVER['REQUEST_METHOD'] ) ) {
			// Source: https://github.com/AdvancedCustomFields/acf/blob/master/includes/rest-api/class-acf-rest-request.php#L89
			// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.MissingUnslash -- Approved inside ACF, not our code.
