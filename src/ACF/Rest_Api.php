...
use WPConnectr\ACF\Rest_Request;
use WP_Term;

defined('ABSPATH') || exit;

if ( !class_exists( 'ACF_Rest_Api' ) ) {
	// Our class is a clone of the ACF_Rest_Api class from the ACF plugin.
	// We need to copy the entire class here because the ACF_Rest_Api class uses private methods and variables we cannot access.
	// We also need to make some modifications to the class to make it work with our plugin.

	// Copied from version 6.3.6 of the ACF Pro plugin.
	return;
}
...
class Rest_Api {
	...
	public function __construct() {
		...
		add_action( 'rest_api_init', array( $this, 'register_field' ), 2 );

		add_filter( 'acf/rest/get_field_schema', array( $this, 'get_field_schema' ), 10, 2 );
	}
	...
	/**
	 * ACF does not provide context for fields in the REST API. This method allows us to add context.
	 *
	 * @param array $schema
	 * @param array $field
	 * @return array
	 */
	public function get_field_schema( $schema, $field ) {
		// remove 'null' if it appears in $schema['type']
		if ( ! isset( $schema['type'] ) ) {
			$schema['type'] = array();
		}

		if ( is_array( $schema[ 'type' ] ) ) {
			$schema['type'] = array_filter( (array) $schema['type'], function ( $type ) {
				return 'null' !== $type;
			} );
		}
	...
	public function register_field() {
		...
		if ( ! $this->request instanceof Rest_Request ) {
			$this->request = new Rest_Request();
		...
		Logger::get_instance()->info( 'Registering ACF REST field for ' . $base );
		...
		// Safe here because our route will not be registered by acf
	...
	public function update_fields( $data, $object, $property, $request, $object_sub_type ) {
		...
		if ( ! $object_id or ! $object_type ) {
			return new \WP_Error(
				...
				// phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText -- ACF is source of this
		...
		foreach ( $field_group['location'] as $rule_group ) {
			...
			foreach ( $rule_group as $rule ) {
				...
				if ( 'post' === $object_type && 'post_type' === $rule['param'] ) {
					if ( '==' === $rule['operator'] && $rule['value'] !== $this->request->object_sub_type ) {
						continue;
					}
					if ( '!=' === $rule['operator'] && $rule['value'] === $this->request->object_sub_type ) {
						continue;
				...
				if ( in_array( $object_type, array( 'user', 'comment' ), true ) ) {
					$match = true;
				}
			}
	...
	private function get_field_groups_by_id( $object_id, $object_type, $object_sub_type = null, $scope = array() ) {
		...
		switch ( $object_type ) {
			...
			case 'comment':
				...
				if ( ! $comment ) {
					// TODO: Support ACF fields for comments by allowing post_type as a parameter to specify the comment type in incoming requests.
					return array();
				}
		...
		return array_filter(
			...
			function ( $group ) use ( $scope ) {
				if ( $scope && ! in_array( $group['key'], $scope, true ) ) {
					return false;
				}
